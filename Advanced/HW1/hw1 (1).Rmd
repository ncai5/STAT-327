...
```{r}
#question a
plot(y = area$farm, x = area$land, xlab = "land area", ylab = "farm area")
result1 <- lm(farm ~ land, data = area)
abline(result1, col ="limegreen")
# question b
k = 19000
tukey.gr = function(par){
  b0 = par[1]
  b1 = par[2]
  input = area$farm - b0 -b1*area$land
  p0 = NULL
  p1 = NULL
  for (i in (1:length(input))){
    if (abs(input[i]) > k){
      p0[i] = 2*k*sign(input[i])
      p1[i] = p0[i]*area$land[i]
    }
    else {
      p0[i] = 2*input[i]
      p1[i] = p0[i]*area$land[i]
...
result2 = optim(par = c(0,0), fn=tukey, method = "Nelder-Mead")
abline(a =result2$par[1], b =result2$par[2], col = "navy")
#question c
result3 = optim(par = c(0,0), fn=tukey, gr = tukey.gr, method = "BFGS")
abline(a =result3$par[1], b =result3$par[2], col = "black")
#question d
result4 = optim(par = c(0,0), fn=tukey, gr = tukey.gr, method = "CG")
abline(a =result4$par[1], b =result4$par[2], col = "coral", lty = "dashed")
#question e
legend(x="topleft", legend=c("Least square", "Nelder-Mead", "BFGS", "CG"),
       lty=c(1,1,1,2), col=c("limegreen","navy","black","coral"))
```

f. For which method was the value of the Tukey function the smallest?

Nelder-Mead method gives the smallest value of Tukey function.
...
curve(expr = pfunction, from = -100000, to = 100000)
...
predict = function(b){
  n = length(nhtemp)
  nhtemp.hat = NULL
  nhtemp.hat[1] = nhtemp[1]
  for(i in (2:n)){
    nhtemp.hat[i] = b*nhtemp[i-1] + (1-b)*nhtemp.hat[i-1]
  }
  return(nhtemp.hat)
}
error = function(b){
  nhtemp.hat = predict(b)
  error = mean((nhtemp.hat[2:60]-nhtemp[2:60])^2)
  return(error)
}
result4 = optimize(error,c(0,1))
result4
cat(seq = "", "The value of the parameter is ", result4$minimum)
```
...
```{r}
nhtemp.hat = predict(result4$minimum)
plot(nhtemp, ylab = "average temperature")
lines(c(1913:1971),nhtemp.hat[2:60], col = "red")
```
...
```{r}
nhtemp.hat1 = predict(0.1)
nhtemp.hat9 = predict(0.9)
plot(nhtemp, ylab = "average temperature")
lines(c(1912:1971),nhtemp.hat, col = "red")
lines(c(1912:1971),nhtemp.hat1, col = "blue")
lines(c(1912:1971),nhtemp.hat9, col = "limegreen")
b.1 = expression(beta == 0.1)
b.9 = expression(beta == 0.9)
legend(x = "topleft", legend = c("original","exponential smoothing", b.1, b.9),
       lty = c(1,1,1,1), col = c("black", "red", "blue", "limegreen"))
```
...
loglikelihood = function(par){
  ...
  x = mtcars$mpg
  n = length(x)
  value = n*log(1/(sqrt(2*pi)*sigma))-(1/(2*sigma^2))*sum((x-mu)^2)
  return(-value)
}
op1 = optim(par=c(1,1), fn = loglikelihood, method = "Nelder-Mead")
cat(seq = "", "The estimated mean is", op1$par[1], ", and the estimated standard error is",op1$par[2],".")
```
...
llike = function(par){
  ...
  b0 = par[1]
  b1 = par[2]
  value = -sum(log(1+exp(b0+b1*x)))+sum(y*(b0+b1*x))
  return(-value)
}
op2 = optim(c(1,1), llike, method = "Nelder-Mead")
x = menarche.cases$age
y = menarche.cases$reached.menarche
plot(jitter(x),y, xlab = "age", ylab = "y", xlim = c(8,18))
points(menarche$Age, menarche$Menarche/menarche$Total, col = "red", pch = 16)
prob = function(x){
  b0 = op2$par[1]
  b1 = op2$par[2]
  value = 1/(1+exp(-(b0+b1*x)))
  return(value)
}
curve(expr = prob, from = min(menarche.cases$age), to = max(menarche.cases$age), add = TRUE, col = "red")
```
