...
#
baby.prop.test = function (x, n, p, conf.level= .95 ) {
  stopifnot((x >= 0) & (x <= n))
  stopifnot(n > 0)
  stopifnot((0 < p) & (p < 1))
  ...
  r$statistic = (p.hat - p) / sqrt(p*(1-p)/n)
  r$p.value=2*pnorm(z,lower.tail = FALSE)
  r$conf.int=c(max(0,p.hat - qnorm(1-(1-conf.level)/2) * sqrt(p.hat*(1-p.hat)/n)),min(1,p.hat + qnorm(1-(1-conf.level)/2) * sqrt(p.hat*(1-p.hat)/n)))
  r$estimate=x / n
  r$null.value=p
  return(r)
}
